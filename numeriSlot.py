...
VITTORIA_SOUND = pygame.mixer.Sound("bonusuono.mp3")
pygame.mixer.Sound.set_volume(VITTORIA_SOUND , 0.7)

SCONFITTA_SOUND = pygame.mixer.Sound("mortesuono.mp3")
pygame.mixer.Sound.set_volume(SCONFITTA_SOUND , 0.5)
...
#         screen.blit(FineGioco, (SCREEN_WIDTH/2 , SCREEN_HEIGHT/2+100))
    ...
    if conta == 20 and tentativo == 0:
        if int(numero1) == int(numero2) and int(numero2) == (numero3):
            pygame.mixer.Sound.play(VITTORIA_SOUND)
            soldi += 10 * int(numero1)
            vittoria += 1
        #elif int(numero1) != int(numero2) or int(numero2) != (numero3):
        else:
            pygame.mixer.Sound.play(SCONFITTA_SOUND)
            soldi -= 1
            vittoria = 0
